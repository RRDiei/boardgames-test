...
const BoardgameThumb = ({ bg }) => {
  return (
    <div className="border-2 rounded-md w-full p-2 mt-3 hover:scale-125 transition-all delay-75 hover:cursor-pointer bg-blue-50">
      <img src={bg.img_url} alt={bg.title} />
      <p className="capitalise text-2xl">{bg.title}</p>
      <div className="flex">
        <div className="w-1/2">
          <div className="mt-2">
            <BsFillPersonFill className="text-2xl inline mr-2" />{" "}
            <span>
              {bg.min_players} - {bg.max_players} players
            </span>
          </div>
          <div className="mt-2">
            <FaClock className="text-2xl inline mr-2" />
            <span>{bg.playtime} minutes</span>
          </div>
        </div>
        <div className="w-1/2">
